...
import { type RequestInfo, type RequestInit } from "../_shims/mod.ts";
import { Response } from "npm:node-fetch";
import { isAssistantMessage } from "./chatCompletionUtils.ts";

type Fetch = (
  req: string | RequestInfo,
  init?: RequestInit,
) => Promise<Response>;
...
function mockChatCompletionFetch() {
  ...
  function handleRequest(
    handler: (
      body: ChatCompletionFunctionRunnerParams<any[]>,
    ...
    return handleRawRequest(async (req, init) => {
      ...
      const body: ChatCompletionFunctionRunnerParams<any[]> = JSON.parse(
        rawBody,
      );
...
function mockStreamingChatCompletionFetch() {
  ...
  function handleRequest(
    ...
    return handleRawRequest(async (req, init) => {
      ...
      const body: ChatCompletionStreamingFunctionRunnerParams<any[]> = JSON
        .parse(rawBody);
...
class RunnerListener {
  ...
  constructor(public runner: ChatCompletionRunner) {
    runner
      ...
      .on(
        "chatCompletion",
        (completion) => this.chatCompletions.push(completion),
      )
      .on(
        "functionCall",
        (functionCall) => this.functionCalls.push(functionCall),
      )
      .on(
        "functionCallResult",
        (result) => this.functionCallResults.push(result),
      )
      .on("finalContent", (content) => (this.finalContent = content))
      .on("finalMessage", (message) => (this.finalMessage = message))
      .on(
        "finalChatCompletion",
        (completion) => (this.finalChatCompletion = completion),
      )
      .on(
        "finalFunctionCall",
        (functionCall) => (this.finalFunctionCall = functionCall),
      )
      .on(
  ...
  async sanityCheck({ error }: { error?: string } = {}) {
    ...
    if (error) {
      ...
      await expect(this.runner.finalFunctionCallResult()).rejects.toThrow(
        error,
      );
    ...
    expect(this.finalChatCompletion).toEqual(
      this.chatCompletions[this.chatCompletions.length - 1],
    );
    expect(await this.runner.finalChatCompletion()).toEqual(
      this.finalChatCompletion,
    );
    expect(this.finalFunctionCall).toEqual(
      this.functionCalls[this.functionCalls.length - 1],
    );
    expect(await this.runner.finalFunctionCall()).toEqual(
      this.finalFunctionCall,
    );
    expect(this.finalFunctionCallResult).toEqual(
      this.functionCallResults[this.functionCallResults.length - 1],
    );
    expect(await this.runner.finalFunctionCallResult()).toEqual(
      this.finalFunctionCallResult,
    );
    expect(this.chatCompletions).toEqual(this.runner.allChatCompletions());
    expect(this.messages).toEqual(
      this.runner.messages.slice(-this.messages.length),
    );
...
class StreamingRunnerListener {
  ...
  constructor(public runner: ChatCompletionStreamingRunner) {
    runner
      ...
      .on(
        "content",
        (delta, snapshot) => this.eventContents.push([delta, snapshot]),
      )
      .on("message", (message) => this.eventMessages.push(message))
      .on(
        "chatCompletion",
        (completion) => this.eventChatCompletions.push(completion),
      )
      .on(
        "functionCall",
        (functionCall) => this.eventFunctionCalls.push(functionCall),
      )
      .on(
        "functionCallResult",
        (result) => this.eventFunctionCallResults.push(result),
      )
      .on("finalContent", (content) => (this.finalContent = content))
      .on("finalMessage", (message) => (this.finalMessage = message))
      .on(
  ...
  async sanityCheck({ error }: { error?: string } = {}) {
    ...
    expect(await this.runner.finalMessage()).toEqual(this.finalMessage);
    expect(this.finalContent).toEqual(
      this.eventContents[this.eventContents.length - 1]?.[1] ?? null,
    );
    expect(await this.runner.finalContent()).toEqual(this.finalContent);
    expect(this.finalChatCompletion).toEqual(
      this.eventChatCompletions[this.eventChatCompletions.length - 1],
    );
    expect(await this.runner.finalChatCompletion()).toEqual(
      this.finalChatCompletion,
    );
    expect(this.finalFunctionCall).toEqual(
      this.eventFunctionCalls[this.eventFunctionCalls.length - 1],
    );
    expect(await this.runner.finalFunctionCall()).toEqual(
      this.finalFunctionCall,
    );
    ...
    expect(this.eventMessages).toEqual(
      this.runner.messages.slice(-this.eventMessages.length),
    );
    if (error) {
      expect(this.error?.message).toEqual(error);
      expect(this.runner.errored).toBe(true);
...
describe("resource completions", () => {
      ...
      await Promise.all([
        ...
        handleRequest(
          async function* (
            ...
            for (
              const choice of contentChoiceDeltas(
                `there are 3 properties in {"a": 1, "b": 2, "c": 3}`,
              )
            ) {
              yield {
                id: "2",
  ...
  describe("stream", () => {
      ...
      await Promise.all([
        handleRequest(
          async function* (
            ...
            for (
              const choice of contentChoiceDeltas(`The weather is great today!`)
            ) {
              yield {
                id: "1",
                choices: [choice],
    ...
    test("toReadableStream and fromReadableStream", async () => {
      ...
      const proxied = ChatCompletionStreamingRunner.fromReadableStream(
